...
public class SelectSearchItemGui extends ModeGui {
    ...
    public Inventory getInventory(ShopMode mode) {
        ...
        inv.setItem(0, ItemUtil.getNamedItem(ItemUtil.getColoredItem(Material.RED_STAINED_GLASS_PANE.name()), ChatColor.RED + "対価で検索", ChatColor.GREEN + "シフトクリック: アイテムの名前で検索"));
        inv.setItem(8, ItemUtil.getNamedItem(ItemUtil.getColoredItem(Material.GREEN_STAINED_GLASS_PANE.name()), ChatColor.GREEN + "商品で検索", ChatColor.GREEN + "シフトクリック: アイテムの名前で検索"));
        inv.setItem(4, ItemUtil.getNamedItem(ItemUtil.getColoredItem(Material.WHITE_STAINED_GLASS_PANE.name()), ChatColor.BLUE + "検索するアイテムを持ってクリック", ChatColor.GREEN + "シフトクリック: NPCの名前で検索"));
