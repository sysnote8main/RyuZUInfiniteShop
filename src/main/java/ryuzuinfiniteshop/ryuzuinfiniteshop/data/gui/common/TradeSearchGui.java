...
public class TradeSearchGui extends TradesGui {
    ...
    public Inventory getInventory(ShopMode mode) {
        Inventory inv = Bukkit.createInventory(new SeachTradeHolder(mode, this, searchedTrade), 9 * 6, ChatColor.DARK_BLUE + "トレード サーチ ページ" + getPage());
        ...
        for(int i = (getPage() - 1) * 6 ; i < getPage() * 6 ; i++) {
            if(i >= trades.size()) {
                ...
                for(int j = 0 ; j < 9 ; j++) {
                    inv.setItem((i % 6) * 9 + j , trades.get(i).getTradeItems(shop.getShopType(), shop.getID())[j]);
                }
            }
        }
